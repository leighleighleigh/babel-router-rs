...
pub trait RoutingSystem: Clone + Debug {
...
cfg_if! {
    if #[cfg(feature = "serde")] {
        pub trait RootData: Debug + Clone + Serialize + DeserializeOwned + Sized {}
        impl<T: Debug + Clone + Serialize + DeserializeOwned + Sized> RootData for T {}
    }
    else{
        pub trait RootData: Debug + Clone + Sized {}
        impl<T: Clone + Sized + Debug> RootData for T {}
    }
}
